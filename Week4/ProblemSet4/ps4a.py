...
def getWordScore(word, n):
    ...
    word_score = []
    for letter in word:
        word_score.append(SCRABBLE_LETTER_VALUES[letter])
    if len(word_score) == n:
        return sum(word_score) * len(word_score) + 50
    else:
...
def updateHand(hand, word):
    ...
    hand_c = hand.copy()
    for letter in word:
        if letter in hand_c.keys():
            if hand_c.get(letter) > 1:
                hand_c[letter] = hand_c.get(letter) - 1
            else:
                 hand_c.pop(letter)
    return hand_c
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        hand_c = hand.copy()
        for k in word_dict.viewkeys() & hand_c.viewkeys():
            if word_dict[k] <= hand_c[k]:
                check.append(1)
            else:
                check.append(0)
        return len(word_dict) == sum(check)
    else:
        return False
